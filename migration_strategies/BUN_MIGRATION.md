...
- **Setup:** `setup.sh` is now the explicit one-time setup path (`bun run setup`) for install + seed. `bun start` only runs lightweight startup checks before launching dev servers.
...
5. **prestart:** `bun start` still runs `prestart`, but `prestart` now performs only lightweight startup checks. Full setup and seeding moved to `bun run setup`.
...
3. **Revert script and config changes**
   ...
   - In `hackerrank.yml`: replace `bun start` / `bun run setup` with `npm start` / `npm run setup`.
